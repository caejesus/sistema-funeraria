  ...
  if (!ativos.length) {
    return (
      <div style={styles.page}>
        <div style={styles.empty}>Nenhum serviço ativo.</div>
      </div>
    );
  }
        ...
        return (
          <div key={item.id} style={styles.card}>
            <div style={styles.header}>
              <div>
                <div style={styles.title}>{item.falecido || form.falecido || "Sem nome informado"}</div>
                {item.numero ? <div>{item.numero}</div> : null}
              </div>

              <div style={styles.status}>{item.status || "Aguardando início"}</div>
            </div>
